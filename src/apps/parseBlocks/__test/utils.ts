...
export const createParserRunner = <T, TContext extends Context>(parser: Parser<T, TContext>) => (ctx: TContext, expected: Omit<Result<T, TContext>, "ctx">) => {
  ...
  if (expected.success) {
    if (!actual.success)
      fail("Expected Success Result. Got Failure");

    expect(actual.value).toEqual((expected as any).value);
  } else {
    if (actual.success)
      fail("Expected Failure Result. Got Success");
...
export function parseParams(params: Record<string, ValueType>): proxima.eth.EventParameter[]  {
  const res: proxima.eth.EventParameter[] = [];
  for (const [key, val] of Object.entries(params)) {
    const ethType = guessEthValueType(val);
    res.push(new proxima.eth.EventParameter(key, ethType, BigNumber.isBigNumber(val) ? val.toString() : val));
  }
  return res;
}

export const events = (events: model.input.events.MangroveEvent[]): Omit<Success<model.input.events.MangroveEvent[], LogParserContext>, "ctx"> => {
  return {
    success: true,
    value: events,
  };
