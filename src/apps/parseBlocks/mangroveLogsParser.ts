...
export const parseMangroveEvents = (): LogParser<model.input.events.MangroveEvent[]> => map(many(any([
...
export const parseMakerBalanceEvent = () => parseLogs<model.input.events.MangroveEvent[]>(["Debit", "Credit"], log => {
...
export const parseOfferListParamsEvents = () => parseLogs<model.input.events.MangroveEvent[]>(_.keys(OfferListParamsEvents), log => {
  return [{
    type: "OfferListParamsUpdated",
    offerList: extractOfferList(log),
    params: {
      active: log.name == OfferListParamsEvents.SetActive ? log.requireParam("value").asBool() : undefined,
      fee: log.name == OfferListParamsEvents.SetFee ? log.requireParam("value").asBigNumber().toString() : undefined,
      gasbase: log.name == OfferListParamsEvents.SetGasbase ? log.requireParam("offer_gasbase").asBigNumber().toString() : undefined,
      density: log.name == OfferListParamsEvents.SetDensity ? log.requireParam("value").asBigNumber().toString() : undefined,
    }
  }];
});
...
export const parseMangroveParamsEvents = () => parseLogs<model.input.events.MangroveEvent[]>(_.keys(MangroveParamsEvents), log => {
  return [{
    type: "MangroveParamsUpdated",
    params: {
      governance: log.name == MangroveParamsEvents.SetGovernance ? log.requireParam("value").asString() : undefined,
      monitor: log.name == MangroveParamsEvents.SetMonitor ? log.requireParam("value").asString() : undefined,
      vault: log.name == MangroveParamsEvents.SetVault ? log.requireParam("value").asString() : undefined,
      useOracle: log.name == MangroveParamsEvents.SetUseOracle ? log.requireParam("value").asBool() : undefined,
      notify: log.name == MangroveParamsEvents.SetNotify ? log.requireParam("value").asBool() : undefined,
      gasmax: log.name == MangroveParamsEvents.SetGasmax ? log.requireParam("value").asBigNumber().toString() : undefined,
      gasprice: log.name == MangroveParamsEvents.SetGasprice ? log.requireParam("value").asBigNumber().toString() : undefined,
      dead: log.name == MangroveParamsEvents.NewMgv ? false : (log.name == "Kill" ? true : undefined),
    }
  }];
});

export const parseApprovalEvents = () => parseLogs<model.input.events.MangroveEvent[]>("Approval", log => {
  return [{
    type: "TakerApprovalUpdated",
    offerList: extractOfferList(log),
    amount: log.requireParam("value").asBigNumber().toString(),
    owner: log.requireParam("owner").asString(),
    spender: log.requireParam("spender").asString(),
  }];
});

export const parseOfferWrittenEvents = () => parseLogs<model.input.events.MangroveEvent[]>("OfferWrite", log => {
  return [{
    type: "OfferWritten",
    offerList: extractOfferList(log),
    maker: log.requireParam("maker").asString(),
    offer: {
      wants: log.requireParam("wants").asBigNumber().toString(),
      gives: log.requireParam("gives").asBigNumber().toString(),
      gasprice: log.requireParam("gasprice").asBigNumber().toString(),
      gasreq: log.requireParam("gasreq").asBigNumber().toString(),
...
export const parseOfferRetractedEvents = () => parseLogs<model.input.events.MangroveEvent[]>("OfferRetract", log => {
  return [{
    type: "OfferRetracted",
    offerList: extractOfferList(log),
 ...
 * @param opts
 */
export const parseOrderExecutionEvents = (opts: { allowEmptyOrder: boolean }): LogParser<model.input.events.MangroveEvent[]> => {
  return ctx => {
    ...
    const orderCompletedResult = parseOrderCompletedLog()(takenOffersResult.ctx);
    ...
    return success(
      orderCompletedResult.ctx,
      [{
        type: "OrderCompleted",
        id: `${txHash.toHexString()}-${startIndex}`,
        offerList: extractOfferList(log),
        penalty: log.requireParam("penalty").asBigNumber().toString(),
        takerGave: log.requireParam("takerGave").asBigNumber().toString(),
        takerGot: log.requireParam("takerGot").asBigNumber().toString(),
...
const extractOfferList = (log: proxima.eth.ContractEventPayload): model.input.core.OfferList => {
  return {
...
const parseTakenOffers = (): LogParser<ParsedTakenOffer[]> => ctx => {
  ...
  const posthookFail = parsePosthookFailed()(currentTakenOfferResult.value.id)(restOffersResult.ctx);
...
const parsePosthookFailed = () => (requestedOfferId: model.input.core.OfferId) =>
  parseLogsIf(
    "PosthookFail",
    log => log.requireParam("offerId").asNumber() == requestedOfferId
      ? true
      : `offerId doesn't match`,
    log => true);

const parseOfferSuccess = () => parseLogs<model.input.core.TakenOffer>("OfferSuccess", payload => {
  return {
    id: payload.requireParam("id").asNumber(),
    takerWants: payload.requireParam("takerWants").asBigNumber().toString(),
    takerGives: payload.requireParam("takerGives").asBigNumber().toString()
  }
});

const parseOfferFail = () => parseLogs<model.input.core.TakenOffer>("OfferFail", payload => {
  return {
    id: payload.requireParam("id").asNumber(),
    takerWants: payload.requireParam("takerWants").asBigNumber().toString(),
    takerGives: payload.requireParam("takerGives").asBigNumber().toString(),
    failReason: payload.requireParam("mgvData").asString() as model.input.core.OfferFailReason,
  }
});
...
export function parseLogs<T>(nameOrNames: string | string[], mapFunc: (log: proxima.eth.ContractEventPayload) => T): LogParser<T> {
  return parseLogsIf<T>(nameOrNames, _ => true, mapFunc);
}

export function parseLogsIf<T>(nameOrNames: string | string[], ifFunc: (log: proxima.eth.ContractEventPayload) => true | string, mapFunc: (log: proxima.eth.ContractEventPayload) => T): LogParser<T> {
